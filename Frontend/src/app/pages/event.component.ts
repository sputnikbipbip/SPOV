...
@Component({
  ...
  template: `
    <section class="section event-hero"><div class="container event-hero-grid"><div><span class="badge badge-yellow">20% desconto sócios AEVPORT</span><h1>Os essenciais de Oncologia para Enfermeiros</h1><p>Um dia dedicado a fundamentos, organização de cuidados e atualização aplicada ao contexto de enfermagem veterinária.</p><div class="hero-actions"><a class="button button-light" href="#inscricao">Inscrever-me</a><a routerLink="/socios" class="button button-secondary-light">Tornar-me sócio</a></div></div><div class="event-panel"><span class="eyebrow eyebrow-light">Congresso SPOV 2025</span><app-event-meta [invert]="true" /><img class="section-image section-image-contrast" [src]="posterUrl" alt="Material visual do evento SPOV"></div></div></section>
    ...
    <section class="section section-soft" id="inscricao"><div class="container split-section"><div><span class="eyebrow">Informação útil</span><h2>Local, público e inscrição com acesso direto.</h2><p>Hotel Coimbra Aeminium. Evento dirigido a perfis clínicos, de enfermagem e equipas interessadas em oncologia veterinária.</p><img class="section-image" [src]="posterUrl" alt="Material gráfico SPOV para inscrição"></div><form class="contact-form" action="https://formsubmit.co/azevedo.daniel.se@gmail.com" method="POST" [formGroup]="form" (ngSubmit)="submit()" novalidate><input type="hidden" name="_subject" value="SPOV - Pedido de informação de evento"><input type="hidden" name="_template" value="table"><input type="hidden" name="_next" value="/obrigado"><app-form-notes /><app-form-field label="Nome" name="name" [control]="form.controls.name" placeholder="Nome completo" /><app-form-field label="Email" name="email" type="email" [control]="form.controls.email" placeholder="nome@exemplo.pt" error="Indique um email válido." /><app-textarea-field label="Observações" name="notes" [control]="form.controls.notes" placeholder="Pedidos de informação, perfil ou questões práticas" /><button type="submit" class="button button-primary">Pedir informação</button></form></div></section>
  `
})
